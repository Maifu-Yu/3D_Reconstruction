...
class Oxyz:
    def __init__(self, folder_path, csv_file_path): # folder_path是图片文件夹的路径，csv_file_path是csv文件的路径
        ...
        self.coordinate = np.zeros((200, 200, 200)) # WHS Rat 39 um  x=511, y=1023, Z=511  ABA Mouse 25 um  X=455, y=527, z=319
    ...
    def RotationCalculation(self, x_2D, y_2D):
        global x, y, z
        # 计算新的坐标系
        x = int((ox + (ux * x_2D)/self.width + (vx * y_2D)/self.height)/3.99)
        y = int((oy + (uy * x_2D)/self.width + (vy * y_2D)/self.height)/3.99)
        z = int((oz + (uz * x_2D)/self.width + (vz * y_2D)/self.height)/3.99)
        # 这里的3.99是100µm的缩放比例，应该提供选项
        return x, y, z
            ...
            if filename.endswith(".png"):
                ...
                for x_2D in range(self.height):
                    for y_2D in range(self.width):  # 降噪也可以加在这里
                        # if self.img[x_2D, y_2D] > 100:  # 降噪，暂时关闭了，这里应该设置一个布尔值供用户选择是否降噪的
                            # print(x_2D, y_2D)
