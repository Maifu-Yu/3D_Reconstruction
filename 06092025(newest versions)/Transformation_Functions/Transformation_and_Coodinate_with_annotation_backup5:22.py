...
class Oxyz:
    def __init__(self, folder_path, csv_file_path, anotation_path): # folder_path是图片文件夹的路径，csv_file_path是csv文件的路径
        ...
        self.coordinate = np.zeros((550, 550, 550, 2)) # WHS Rat 39 um  x=511, y=1023, Z=511  ABA Mouse 25 um  X=455, y=527, z=319
            ...
            if filename.endswith(".png"):
                ...
                for x_2D in range(self.height):
                    for y_2D in range(self.width):  # 降噪也可以加在这里
                        if self.img[x_2D, y_2D] > 50:  # 降噪，暂时关闭了，这里应该设置一个布尔值供用户选择是否降噪的
                            # print(x_2D, y_2D)
                            self.RotationCalculation( x_2D,  y_2D)
                            # self.coordinate[(527-y),(319-z),x, 0] = self.img[x_2D, y_2D] # save the new coordinate
                            self.coordinate[x,y,z, 0] = self.img[x_2D, y_2D]
                        # 坐标换算成Allen CCFv3的坐标体系
                            ...
                            try:
                               # self.coordinate[(527-y),(319-z),x, 1] = self.ANO[(527-y),(319-z),x]  # Mingyang好像是z,y,x.这里需要查证一下  https://www.nitrc.org/plugins/mwiki/index.php?title=quicknii:Coordinate_systems
                                self.coordinate[x,y,z, 1] = self.ANO[x,y,z]
                            except:
                                self.coordinate[x,y,z, 1] = 0
